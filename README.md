...
4. nms진행시 오리지널 이미지의 box score우선하여 ios계산하는 방식 진행 - 박스를 너무 많이 삭제하고, 대체로 정확도가 높은 오리지널 이미지 detect box도 삭제됨
5. 위의 단계에서 ios계산 후 오리지널 이미지의 box데이터 살리는 방식 진행(origin-nms) - 어느정도 개선 됐으나 오탐박스가 해결되지 않음
