...
from sahi_improve.sahi.sahi.postprocess.utils import ObjectPredictionList, has_match, merge_object_prediction_pair
from sahi_improve.sahi.sahi.prediction import ObjectPrediction
from sahi_improve.sahi.sahi.utils.import_utils import check_requirements
...
def batched_nms(predictions: torch.tensor, match_metric: str = "IOU", match_threshold: float = 0.5,len_original:int=0):#**
    """
    ...
    #first, only slices image data nms, after with original image data origin-nms
...
def origin_nms(
    ...
    #Original id Adjustment where add back slice ID
    ...
    #if original_order value not in keep, add
...
class NMSPostprocess(PostprocessPredictions):
    def __call__(
        ...
        self.len_original=len_original
